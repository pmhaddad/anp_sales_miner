...
def test_make_path(tmp_path):
    dummy_datalake = tmp_path / 'datalake'
    dummy_datalake.mkdir()
    os.environ['DATALAKE_PATH'] = str(dummy_datalake)

    path = make_path(stage='a_dummy_stage', table='a_table_name')

    assert path == f'{tmp_path}/datalake/a_dummy_stage/a_table_name'


def test_make_path_with_parents_error(tmp_path):
    os.environ['DATALAKE_PATH'] = 'with/parents/datalake'
...
def test_make_path_exists_ok_no_error(tmp_path):
    dummy_datalake = tmp_path / 'datalake'
    dummy_datalake.mkdir()
    os.environ['DATALAKE_PATH'] = str(dummy_datalake)
